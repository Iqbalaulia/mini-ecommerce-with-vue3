...
<template>
	<div class="product-detail">
		<div class="row">
          ...
          <hr>
				...
				<div class="price">
					<label for="">
						{{
							formatCurrency(stateProductDetail.dataDetail &&
        ...
        <div class="description">
          ...
          <p>
            {{
              stateProductDetail.dataDetail && stateProductDetail.dataDetail.description
            }}
          </p>
        </div>
        <div class="merchant">
          <hr>
            <div class="information">
              <img class="img-fluid" v-once src="../../../../assets/logo/logo-superindo.webp" alt="">
              <div class="merchant-name">
                <label for="">
                  <fa :icon="['fas', 'circle-check']" class="mr-1 icon-check" />
                  {{ 	stateProductDetail.dataDetail && stateProductDetail.dataDetail.merchant }}
                </label>
        ...
        <div class="transaction">
          ...
          <div class="input-selling">
            <button class="btn btn-sell" @click="funcMinus()" :class="qtyProduct === 1 ? `disabled` : ``">
              <fa :icon="['fas', 'minus']" class="mr-1" />
            </button>
            <input class="sell" type="text" v-model="qtyProduct">
            <button class="btn btn-sell"  @click="funcPlus()" :class="stateProductDetail.dataDetail.stock === 0 ? `disabled` : ``">
          ...
          <div class="subtotal">
            ...
            <div class="rupiah">
              {{formatCurrency(stateProductDetail.dataDetail && stateProductDetail.dataDetail.price * qtyProduct)}}
            </div>
          </div>
          <button class="btn btn-primary-superindo w-100 mb-3 mt-3">
            + Tambah Keranjang
          </button>
        </div>
      </div>
...
export default defineComponent({
	...
	methods: {
		...mapActions("stateProductDetail", ["getDataDetailProduct"]),
	},
	mounted() {
		this.getDataDetailProduct({ uuid: this.$route.params.id });
	},
